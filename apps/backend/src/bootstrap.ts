 ...
 * bootstrap.ts
 *
 * Runs BEFORE the main application. Fetches secrets from AWS Secrets Manager
 * and populates process.env so that config.ts (which validates env vars at
 * import time via Zod) and Prisma (which reads DATABASE_URL at startup) both
 * find all required values.
 *
 * Loaded via CMD ["node", "dist/bootstrap.js"] in the Dockerfile.
 */
...
async function loadSecrets(): Promise<void> {
  ...
  // ── 2. Load app secrets (API keys, JWT_SECRET, etc.) ────────────────────
...
loadSecrets()
  .then(() => {
    // All env vars are now in place — load and run the main application.
    // Using require() (not import) so module evaluation is deferred until here.
